...
use crate::{gen_etag_header, AssertResponseModifier, Asset};
use bytes::Bytes;
...
fn headers<A: Asset>(asset: &A) -> http::response::Builder {
    http::Response::builder()
        .header(http::header::CONTENT_TYPE, HeaderValue::from_str(asset.mimetype()).unwrap())
        .header(http::header::CONTENT_LENGTH, asset.len())
        .header(http::header::ETAG, gen_etag_header(asset))
}

pub(crate) fn head<A: Asset>(asset: &A, path: &str, f: &impl AssertResponseModifier) -> ResponseFuture {
    let response = f.modify(path, headers(asset));
    to_response(response.body(empty_body()))
}

pub(crate) fn not_modified<A: Asset>(asset: &A, path: &str, f: impl AssertResponseModifier) -> ResponseFuture {
    let response = f.modify(path, headers(asset)
        .status(http::StatusCode::NOT_MODIFIED));
    to_response(response.body(empty_body()))
}

pub(crate) fn get<A: Asset>(asset: A, path: &str, f: &impl AssertResponseModifier) -> ResponseFuture {
    let response = f.modify(path, headers(&asset));
...
fn err_response(
    ...
    modifier(http::Response::builder().status(status_code))
        .body(empty_body())
}
